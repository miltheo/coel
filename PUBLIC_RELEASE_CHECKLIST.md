...
- [ ] Confirm the local `app/` workspace is ignored and excluded from the public COEL v2.0 release unless it is published separately.
...
- [ ] Confirm model JSON and Turtle files are treated as derived serialisations, not independent normative sources.

## Validation

- [ ] Confirm the COEL Behavioural Atom v2.0 JSON Schema at `atom/2.0/coel-atom.json` matches the release specification.
- [ ] Validate example Atom payloads against the COEL Behavioural Atom v2.0 JSON Schema.
